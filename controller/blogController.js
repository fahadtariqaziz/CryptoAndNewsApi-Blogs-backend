...
const blogController = {
    ...
    async create(req, res,next){
       ...
       //commented line 55,59,66 due to using cloudinary because we save photo on cloud storage not on local storage because on deployement photo cannot be rendered from local storage 
    ...
    async update(req, res, next){
        ...
        if(photo){
            ...
            //paste 
            //const buffer = Buffer.from(photo.replace(), 'base64')  replace karke matlb is part ko remove karke base 64 string se then read buffer.from or 'base64' iski encoding pas kardi
            //commented due to use of cloudinary line 206 210 214 
            //const buffer = Buffer.from(photo.replace(/^data:image\/(png|jpg|jpeg);base64,/,''), 'base64');
